...
# 3b. Build record — what the appliance reports as its identity.
#
...
#     writes the real answer: the commit installed, whether that worktree was
#     clean, and a content hash so a later `tar`/`scp` straight into
#     site-packages (bypassing this installer) shows up as a modified build
#     instead of silently keeping the stale SHA.
#
#     The hash is computed by the package's own compute_content_hash() through
#     the venv python — deliberately NOT reimplemented in bash, because the
#     runtime compares against this value and any divergence between the two
#     implementations would flag every box as modified forever.
...
#     No PYTHONPATH needed (and none wanted — see the unit's design notes):
#     client_agent was just copied INTO site-packages, so the venv python
#     imports it off the default sys.path.
BUILD_HASH="$(
    "$VENV/bin/python" -c \
...
# Generated by install-user.sh — do not edit.
