...
- **⚠ STATION MODE IS DOWN UNTIL THE HEAD IS RETRAINED (since 2026-09-03).** The centre-crop was removed and the rectangle widened to **1670×800 at (1030, 1360)** (client-approved), so `models/station-head-hala-prawe-v1-v1.0.0.*` is fitted on a feature space that no longer exists. `StationCard` now **refuses** any card without `preprocessing.center_crop: false`, which is every card written before this date — a station run fails loudly instead of reporting wrong totals. **Do not run `scripts/deploy-image.sh`** until a re-exported head, card and backbone land; the deployed image still has the old, working-but-blind pair.
- **The head used to see only the MIDDLE 43% of the authored rectangle.** DINOv2's processor resizes the crop to 518 and then centre-crops to 224, so the measured area is `224/518` of each axis — 389×346 px of the 900×800 `hala-prawe-v1` rect, 18.7% of it — while the panel draws the client the whole rectangle and `zone_native_px` reports it. Work that drifts to the edge of the bench leaves the model's view without leaving the client's rectangle, and `coverage` cannot see it: coverage counts samples predicted, not whether the activity was in frame. This produced a production run reporting `spawanie: 0.0 s` over an hour of welding (task `2240728f`, 2026-09-03).
