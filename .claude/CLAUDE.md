...
- Station chronometraż (`--classifier station`, #123): answers "how much of the session went on welding / laying rods / everything else" at ONE workstation. The zone is the unit, not a person — frozen DINOv2 backbone + a ~1 MB station head over a native-frame zone crop at the card's 2 s stride, and it constructs **no pose session, no OSNet and no VLM** (asserted by test). Needs `--zones` naming exactly one zone with `rules.type: "station"` whose polygon bbox equals the card's `station.zone_native_px`, or it refuses. Adds an **additive** `station_activity` section to `result.json`; `schema_version` stays 6 and every other mode's output is byte-identical (golden-file test). Selected per camera by `classifier: "station"` in the mounted `zones.json`, resolved at container start like `pose.mode`. Measured on cctv-vps: **521 GPU-s/video-hour, 754 MiB peak** — of which decode is 386 and the model work only 39, so it is decode-bound like every other mode on 4K (`benchmark-results/issue-123`).
- **v3.0.0 head, fitted on the current bench layout (2026-09-04).** Trained on five annotated windows, two of them (W4, W5) cut from the appliance buffer after the bench was rearranged. Cross-validated over five folds: `spawanie` **91.5%** recall at 1.02× time, `ukladanie_pretow` 82.2% at 0.91×. Rectangle, tensor and vocabulary are unchanged from v2.0.0.
- **What the retrain was worth, and how it was measured.** The ablation held the test window fixed and moved only the training set: train W1–W3 → W5 gives `spawanie` **56.2%**; train W1–W4 → W5 gives **92.1%**. So one 20-minute current-layout window buys 36 points of recall, and reported welding time goes from 56% of the truth to 94%. An independent production run of v2.0.0 over held-out W4 landed at 56.5% — the same number from a different direction, which is what settled that the cause was domain shift rather than geometry.
- **v1.0.0 saw only the MIDDLE 43% of the rectangle, and nobody chose that.** DINOv2's processor takes `size` and `crop_size` independently; training set `size=518` for a high-resolution view and left `crop_size` at its 224 default, so the head read 389×346 px of the 900×800 rect — 18.7% of the area — while the card, the panel and `zone_native_px` all reported the whole thing. `coverage` cannot catch it: it counts samples predicted, not whether the activity was in frame. `StationCard` now **refuses** any card without `preprocessing.center_crop: false`, and `model_input_for_rect` derives the tensor from the rectangle in one place used by the exporter, the training embedder and the card generator — two independent implementations of one convention is what let both carry the same unintended crop.
- **⚠ Retraining is the fix for a rearranged bench; geometry is not.** Widening the rectangle (v2.0.0) improved every cross-validated figure and still reported `spawanie: 0.0 s` on the incident footage. When a station's numbers collapse, ask first whether the scene changed since the last training window — and note the arc guard bounds only the catastrophic case: it returned `consistent` on a run that found 57% of the welding, because it fires on a head reporting almost none.
- **Two gaps v3.0.0 does not close.** Neither current-layout window contains an empty station, so `brak_na_stanowisku` and `nierozpoznane` are still learned only from pre-rearrangement footage. And W4/W5 are the same session 50 minutes apart, so holding one out tests another hour rather than another day.
- **A report names the head that measured it, and the panel refuses a withdrawn one.** `station_activity.zones[].model.version` is compared against the catalogue's current head; a mismatch withholds the totals instead of captioning them. Reports cannot be recomputed — retention reaps a completed task's input immediately — so withholding is the only honest treatment.
- **Arc-flash guard** (`pipeline/arc_flash.py`): every station run now measures the fraction of the **whole** rectangle that is clipped-bright *and* blue-dominant, and emits `station_activity.zones[].arc_check`. Verdict `contradicted` (sustained, spread-out arc evidence against a head that reported almost no welding) sets `reliable: false` on **every** category in that zone — the missed seconds were counted as something else, so the whole split moved. A presence test, never a duration: it never edits a total. Thresholds are calibrated against the three annotated windows plus the incident hour, not guessed — see the module docstring.
...
- **Station chronometraż** (#123, one zone): **521 GPU-s/video-hour** (6.9:1), **754 MiB VRAM** (`benchmark-results/issue-123`). Decode-bound, not model-bound: ffmpeg 386 s/video-hour vs 39 for preprocess + backbone. The `49 GPU-s/video-hour` figure in #120/#123 is the benchmark's embedding pass over pre-extracted crops and excludes video decode — both numbers are right, they measure different things. Sampling at the 2 s stride does not decode less (381 vs 386): ffmpeg's `fps` filter drops frames after decoding them. Cutting this means NVDEC in `video_frames.py`, which would help every mode.
