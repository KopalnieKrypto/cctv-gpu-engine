...
- **v2.0.0 head, widened rectangle, no centre-crop (2026-09-03).** The rectangle is **1670×800 at (1030, 1360)** (client-approved against a live frame) and the whole of it now reaches the backbone as a **1078×518** tensor. `StationCard` **refuses** any card without `preprocessing.center_crop: false` — every card written before this date — so a stale head fails loudly instead of scoring pixels at a framing it was never fitted on. `model_input_for_rect` derives the tensor from the rectangle in one place used by the exporter, the training embedder and the card generator; two independent implementations of that convention is what let both carry the same unintended crop.
...
- **⚠ The widening did NOT fix the incident, and the cause is domain shift, not geometry.** v2.0.0 improves cross-validated `spawanie` (89.0% → 92.7% recall at 1.06× time) and reports 461 s of welding on W1 against 447 s of truth — the chain works. It still reports **`spawanie: 0.0 s`** on 2026-09-03 footage, which carries *more* arc evidence than W1 (7 flagged samples in 331 vs 4 in 599). The bench was rearranged after the last training window (2026-09-01), and the head learned the old layout. **Fixing this needs annotated material from the current layout** — no geometry or preprocessing change will do it. Until then the arc guard marks such runs `contradicted` and the panel withholds the totals.
