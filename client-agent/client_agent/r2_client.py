...
to the caller. Status-list walks share an ETag-keyed cache so an unchanged
bucket costs zero ``get_object`` calls per poll.
"""
...
def _with_retry(op: Callable[[], T]) -> T:
    ...
    that already translate boto3 errors (web UI → 404/500) keep working
    unchanged. ``time.sleep`` is looked up on the module at call time so tests
    can patch it and assert the backoff schedule.
    """
...
class R2Client:
    ...
    def __init__(
        ...
        # poll — an idle 1000-job bucket otherwise burns ~8.6M reads/day.
