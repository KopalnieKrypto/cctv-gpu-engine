"""Boundary tests for the client-agent's boto3-backed R2 client.

This is the client-agent's owned copy of the R2Client. The historical
``gpu_service.r2_client`` lives in the gpu-service package and is exercised
by its own test suite; the client-agent package owns this one so the
appliance no longer needs gpu_service installed (issue #40).

Test strategy mirrors ``gpu_service/r2_client_test.py``: each test wires a
fresh ``MagicMock`` as the ``boto3.client`` return value and asserts both
the calls made to it (key naming conventions from SPEC §6.2) and the values
returned by R2Client. Mocking happens at the boto3 boundary; the R2Client
itself is exercised through its real public methods.
"""
...
def _make_client(s3: MagicMock) -> R2Client:
    ...
    with patch("client_agent.r2_client.boto3.client", return_value=s3):
        return R2Client(
...
def test_constructor_creates_boto3_s3_client_with_r2_endpoint() -> None:
    ...
    """
    with patch("client_agent.r2_client.boto3.client") as boto3_client:
        R2Client(
...
def test_get_report_returns_result_json_bytes_from_output_result_json_key() -> None:
    """``get_report`` reads the canonical result.json the worker wrote to R2
    and returns its raw bytes — consumed by the client-agent's report viewer.

    Issue #74: #72 retired ``output/report.html`` in favour of the structured
    ``surveillance-jobs/{job_id}/output/result.json`` artifact (mirrors the
    gpu-service R2Client). Artifacts are small enough (structured data + base64
    JPEG frames) that an in-memory read is fine — no streaming required here.
    """
    s3 = MagicMock()
...
def test_upload_report_uses_output_result_json_key_and_returns_it() -> None:
    """Issue #74: the report artifact is now ``output/result.json`` written
    with ``application/json`` (mirrors the gpu-service R2Client after #72)."""
    s3 = MagicMock()
    client = _make_client(s3)
